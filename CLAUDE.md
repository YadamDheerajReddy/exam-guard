...
Env vars: `web/.env.example` lists the required Supabase + `BARCODE_TOKEN_SECRET` + `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD` vars for `web/.env.local`. Mobile reads `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_ANON_KEY`, and `EXPO_PUBLIC_API_BASE_URL` from `mobile/.env` — the last one must be the machine's LAN IP (not `localhost`) when testing on a physical phone via Expo Go, since `localhost` from the phone resolves to the phone itself.
