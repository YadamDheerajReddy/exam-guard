...
import { redeemInvigilatorPasswordReset } from "./actions";
import { AlertCircle, CheckCircle2, KeyRound } from "lucide-react";

export function ResetPasswordForm({ token }: { token: string | null }) {
  const [state, formAction, pending] = useActionState(redeemInvigilatorPasswordReset, undefined);
    ...
    <div className="w-full max-w-sm animate-in fade-in slide-in-from-bottom-2 rounded-lg border border-border bg-white p-8 shadow-sm duration-300">
      ...
      {!token ? (
        <p className="mt-6 flex items-center gap-2 rounded-lg bg-alert-tint px-3 py-2 text-sm text-alert">
          ...
          This reset link is missing its token. Request a new one from the scanner app.
        </p>
      ) : state?.ok ? (
        <p className="mt-6 flex items-start gap-2 rounded-lg bg-verified-tint px-3 py-3 text-sm text-verified">
          <CheckCircle2 className="mt-0.5 size-4 shrink-0" strokeWidth={2} />
          Password updated. Return to the ExamGuard Invigilator app and sign in with your new password.
        </p>
      ) : (
